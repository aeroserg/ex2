...
$element = CIBlockElement::GetList(
    ...
    false,
    false,
    array('ID', 'META_TITLE', 'META_DESCRIPTION')
)->Fetch();



if ($element) {
	$newel =  CIBlockElement::GetPropertyValues(
		$iblockId, 
		$element, 
			false, 
			array('ID' => array(9, 10))
	...
	if ($newel) {
		$newel['META_TITLE'] = $newel[9];
		$newel['META_DESCRIPTION'] = $newel[10];
		$APPLICATION->SetPageProperty('title', $newel['META_TITLE']); 
		$APPLICATION->SetPageProperty('description', $newel['META_DESCRIPTION']); 
	}
}
?>
<!DOCTYPE html>
