...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["submit"] <> '' && (!isset($_POST["PARAMS_HASH"]) || $arResult["PARAMS_HASH"] === $_POST["PARAMS_HASH"]))
{
	...
	if(check_bitrix_sessid())
	{
		...
		if($USER->IsAuthorized()) {
			$authorMessage = $USER->getId()." (".$USER->GetLogin().") ".$USER->GetFullName().", " . $_POST["user_name"];
		} else {
			$authorMessage = $_POST["user_name"];
		}
			$arFields = Array(
...
AddMessage2Log("«Замена данных в отсылаемом письме – ", $authorMessage);
if($arParams["USE_CAPTCHA"] == "Y")
